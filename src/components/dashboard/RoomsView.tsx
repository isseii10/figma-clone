...
import { a } from "node_modules/@liveblocks/react/dist/room-Ce3twcjb.mjs";
    ...
    <div ref={outerDivRef} className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap gap-4">
        {filteredRooms.map((room) => {
          const roomColor =
            roomColors.find((r) => r.id === room.id)?.color ??
            PASTEL_COLORS[0]!;
          return (
...
const SingleRoom = ({
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Backspace" && selected && !isEditing) {
        event.preventDefault();
